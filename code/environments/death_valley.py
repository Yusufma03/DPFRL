...
class DeathValleyEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(-1., 1., shape=(2+self.noise_length,))
        self.viewer = None
    ...
    def observation(self):
        ...
        if self.noise_length > 0:
            noise_vec = np.random.uniform(-1, 1, self.noise_length)
            rand_obs = np.concatenate((rand_obs, noise_vec))
    ...
    def get_reward(self, X, Y):
        ...
        for mean, cov, factor in zip(means, covs, factors):
            ...
            Z = np.maximum(Z, factor * bivariate_normal(X, Y, cov[0, 0], cov[1, 1], mean[0], mean[1], cov[0, 1]) / num_mixtures)
    ...
    def _reset(self):
        if self.mode == 'single':
            ...
            ori_seed = np.random.get_state()
            np.random.seed()
            start = self.starting_positions[np.random.randint(len(self.starting_positions))]
            np.random.set_state(ori_seed)
        else:
